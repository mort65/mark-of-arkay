...
Bool function bIsRespawnForbidden(Location akLoc)
  ...
  if ConfigMenu.bPUOK
    if JsonUtil.JsonExists("/MarkofArkay/MOA_BlackLists")
      ...
      iIndex = locArr.Length
      while iIndex > 0
        iIndex -= 1
        if locArr[iIndex] As Location
    ...
    iIndex = ReviveScript.LocationBlackList2.GetSize()
    while iIndex > 0
      iIndex -= 1
      ForbiddenLocation = ReviveScript.LocationBlackList2.GetAt(iIndex) As Location
      if ForbiddenLocation
        if (akLoc == ForbiddenLocation) || ForbiddenLocation.IsChild(akLoc)
          return True
        endif
